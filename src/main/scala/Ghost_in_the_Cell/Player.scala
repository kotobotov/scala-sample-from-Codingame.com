...
//                Answer.add(s"MOVE $id ${shorterPath(to)} $amount")
        ...
        def shorterPath(to: Int): Int = {
            val oldDistance = listOfNodes.find(item => (item._1 == to)).map(_._2).head
            val betterWayNodes = listOfNodes.map(cellId => cells(cellId._1))
                    .filter(_.owner != Config.OPPONENT)
            val numbersOfPaths = betterWayNodes.size

            def step(counter: Int): Int = {
                betterWayNodes(counter).listOfNodes
                        .find(item => (item._1 == to) && (item._2 < oldDistance)) match {
                    case Some(x) =>
                        betterWayNodes(counter).id
                    case _ =>
                        if (counter < numbersOfPaths - 1) step(counter + 1) else to
                }
            }
           try{ step(0)}
            catch {case e:Exception => to}
        }
...
//                    getROI(item.get._2.toDouble))
    ...
    def mainStrategy = {
        val otvet = cells.filter(cell => cell.owner == Config.ME)
                .foreach(current => {
                    current.defend
                    current.targetList.foreach(target => current.attack(target._1))
